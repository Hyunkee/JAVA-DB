...
public class StdManager {
	 ...
	 * 		 추가하고 아니면 추가하지 않으며 추가 했을 경우 성공을, 추가하지 못하면 실패를 알려주는 기능
	 * 매개변수 : 학생정보 => Std s
	 * 리턴타입 : boolean
	 * 메소드명 : insert
	 * */
	public boolean insert(Std s){
		/* 매개변수 s를 그대로 넣으면 list와 외부에서 s를 같이 생성
		 * s를 복사생성자를 이용하여 s의 값과 동일한 값을가지는 tmp 객체를 하나 만든 후 해당 tmp를 해당 리스트에 추가한다.		 * 
		 * */
		Std tmp = new Std(s);
		/* add()라는 메소드는 list에 tmp와 일치하는 정보가 없으면
		 * 해당 객체를 추가하고 true를 리턴하며 tmp에 일치하는 정보가 있으면 추가하지 않고
		 * false를 리턴한다.(Set의 특징)*/
		return list.add(tmp);		
	}
	...
	public Std search(Std s){
		...
		while(it.hasNext()){
			Std tmp = (Std)it.next();			
			/* list에서 꺼낸 tmp와 s가 같은지를 equals를 이용하여
			 * 비교한 후 같으면 꺼낸 tmp를 반환
			 * */
			if(tmp.equals(s))
				return tmp;
		}
		// 반복문이 종료될때까지 return이 안된 것은 해당 정보와 일치하는 객체가 list에 없는 경우 이기 때문에
		// null 값을 리턴
		return null;
	}
	
	/* 기능 : 학생정보가 주어지면 list에서 해당 학생의 정보를 삭제하는데 일치하는 학생정보가 없으면
	 *       실패를, 있으면 삭제를 해서 성공을 알려주는 기능
	 * 매개변수 : 학생정보 => Std s
	...
	public boolean delete(Std s){
		Std tmp = search(s);
		/* search()를 통해 검색했을 때 일치하는 정보가 있으면 객체 정보가
		 * 없으면 null을 전달한다.
		 * remove()는 tmp라는 객체와 일치하는 정보가 있으면 해당 정보를
		 * 삭제한 후 true의 값을 리턴하고, 없으면 false값을 리턴한다.
		 * */		
	...
	public boolean update(Std s){
		/* set에서 수정을 하려면 기존에 있는 객체를 삭제 후 수정된 객체정보를
		 * 추가해야 한다.
		 * */
		if(!delete(s))
			return false;
	 ...
	 * 매개변수 : 없다 //전체학생을 출력하기 때문에 매개변수가 필요없다.
	 * 리턴타입 : 없다 => void
	 * 메소드명 : print
	 */
	public void print(){
		Iterator<Std> it = list.iterator();		// 암기
		while(it.hasNext()){					// 암기
			Std tmp = (Std)it.next();			// 암기
