...
public class ArrayExam1 {
	public static void main(String[] args){
		...
		/*
		 * 자료형 [] 배열이름; // 배열을 선언
		 * 배열이름 = new 자료형[배열의 크기]; // 배열을 생성
		 * 자료형 배열이름[]; // 배열을 선언
		 * 배열이름 = new 자료형[배열의 크기]; // 배열을 생성
		 * 배열의 시작 번지는 0번지 부터 이다.
		 * 배열의 마지막 번지는 배열의 크기 - 1 번지까지 이다.
		 * 배열은 주로 반복문과 사용된다
		 * 잘못된 번지로 접근하면 예외가 발생한다. =>ArrayIndexOutOfBoundsException
		...
		int [] kor;
		kor = new int[10]; // int [] kor = new in[10];
		kor[0] = 10;
